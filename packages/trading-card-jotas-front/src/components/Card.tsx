...
export function Card({
  ...
  return (
    <TripleBorder borderColor={borderColor} className="w-full h-full">
      <div
        className={`flex justify-start pt-1 pb-1 h-full items-center flex-col gap-1 ${
          facingDown ? "text-primary bg-primary" : ""
        } ${className}`}
      >
        {!facingDown && (
          <>
            <img
              draggable={false}
              className="w-[calc(100%-10px)] aspect-square select-none"
