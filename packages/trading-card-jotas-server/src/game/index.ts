import * as CardsObject from "../../../trading-card-jotas-types/cards";
...
const onReveal = (
  attackingPlayer: ConnectedSocket,
  defendingPlayer: ConnectedSocket
) => {
  ...
  const cards = [
    {
      card: CardsObject[attackingPlayer.cardStack.slice(-1)[0].cardKey].default,
      owner: attackingPlayer,
      id: attackingPlayer.cardStack.slice(-1)[0].id,
    },
    {
      card: CardsObject[defendingPlayer.cardStack.slice(-1)[0].cardKey].default,
      owner: defendingPlayer,
      id: defendingPlayer.cardStack.slice(-1)[0].id,
    },
  ];

  const [{ card: firstCard, owner: firstCardOwner, id: firstCardId }] = cards
    .sort((a, b) => (b.card.priority ?? 0) - (a.card.priority ?? 0))
    .splice(0, 1);
  const [{ card: otherCard, owner: otherCardOwner, id: otherCardId }] =
    cards.splice(0, 1);
  ...
  if (
    firstCardOwner.cardStack.slice(-1)[0].id === otherCardId ||
    otherCardOwner.cardStack.slice(-1)[0].id === firstCardId
  )
    otherCard.effect(firstCardOwner, otherCardOwner);
  else otherCard.effect(otherCardOwner, firstCardOwner);

  const attackingPlayerStack = attackingPlayer.cardStack.map(
    ({ cardKey }) => CardsObject[cardKey].default
  );
  const defendingPlayerStack = defendingPlayer.cardStack.map(
    ({ cardKey }) => CardsObject[cardKey].default
  );
  ...
  attackingPlayer.send(
    `loadOtherStack/${JSON.stringify(defendingPlayer.cardStack)}`
  ); //TODO dry?
  defendingPlayer.send(
    `loadOtherStack/${JSON.stringify(attackingPlayer.cardStack)}`
  );
...
export const onUserSetCard = (player: ConnectedSocket, card: DeckCard) => {
  if (!player.currentSetCard) {
    ...
    if (roomPlayers.every((player) => player.currentSetCard)) {
      const focusedStack =
        player.stance === "attack" ? otherPlayer.cardStack : player.cardStack;
      const otherStack =
        player.stance === "defense" ? otherPlayer.cardStack : player.cardStack;
