...
type PlayerDeckProps = {
  ...
  rival?: boolean
  onCardMouseDown?: (cardRef: MutableRefObject<HTMLDivElement>) => void
  onCardClick?: (card: CardData) => void
}

type DeckCardProps = Pick<PlayerDeckProps, 'onCardClick' | 'onCardMouseDown'> & { card: DeckCards[number], rival: boolean };

function DeckCard({ card, rival = false, onCardMouseDown = () => { }, onCardClick = () => { } }: DeckCardProps) {
  const cardRef = useRef<HTMLDivElement>(null);

  return (
    <div
      ref={cardRef}
      onMouseDown={() => onCardMouseDown(cardRef as MutableRefObject<HTMLDivElement>)}
      onClick={() => onCardClick((card as CardData))}
      className={`${card.selected ? 'md:-translate-y-3 -translate-y-1' : ''} md:px-1 py-1 px-0 h-full flex items-center cursor-pointer md:hover:-translate-y-3 hover:-translate-y-1`}
    >
...
export function PlayerDeck({ playerSrc, gameData, onCardMouseDown = () => { }, onCardClick = () => { }, deckState: _deckState, rival = false }: PlayerDeckProps) {
  ...
  return (
    <div className={`w-full flex gap-2 items-center ${rival ? 'flex-row-reverse' : ''} md:gap-4`}>
      ...
      <TripleBorder borderColor="gray-light" className="w-full">
        {(gameData?.gameState === 'victory' || gameData?.gameState === 'loss') ? (
          <div className="h-[4.75rem] px-2 py-5">
            <Link href="/" className="group cursor-pointer"><span className="group-hover:visible invisible mr-2">*</span><span>Voltar</span></Link>
          </div>
        ) : (
          <div className="md:flex relative md:gap-0.5 md:justify-start items-center justify-center flex-wrap w-full h-[4.75rem] grid grid-cols-5 gap-1 overflow-y-hidden overflow-x-auto md:overflow-y-visible md:overflow-visible">
            {(rival ? Array.from({ length: 5 }).map((_, i) => ({ borderColor: 'secondary-light', id: i.toString(), src: '' } as DeckCards[number])) : (deck ?? [])).map((card) => (
              <DeckCard key={`${card.id}-inventory`} card={card} rival={rival} onCardClick={onCardClick} onCardMouseDown={onCardMouseDown} />
            ))}
          </div>
        )}
      </TripleBorder>
