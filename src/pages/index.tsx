...
function LoginPage() {
  ...
  const handleLogin = useCallback(() => {
    ...
    signInWithEmailAndPassword(auth, email, password).then((response) => {
      ...
      if (e.code === 'auth/wrong-password') return errorToast('Senha incorreta!')
      if (e.code === 'auth/user-not-found') return errorToast('Não existe usuário com esse email')
  ...
  return (
    <div className='flex w-full h-full flex-col gap-3 items-center'>
      <span className='pl-2 h-1/6 content-center'>Faça login para continuar</span>
        ...
        <div className='flex flex-col justify-between py-6 px-6 gap-10 h-full w-full'>
          <div className='flex flex-col gap-3'>
            <div className='flex flex-col gap-2'>
              <span>Email:</span>
              <Input state={[email, setEmail]} type='email' className='h-min w-full' />
            </div>
            <div className='flex flex-col gap-2'>
              <span>Senha:</span>
              <Input state={[password, setPassword]} type='password' className='h-min w-full' />
            </div>
          </div>

          <div className='flex flex-col gap-4 items-center justify-end text-center w-full'>
            <button disabled={loading} onClick={handleLogin} style={{ ...pixelBorder('black'), boxShadow: `inset 0 0 0 2px black` }} className='px-2 py-0.5 bg-white  disabled:bg-gray disabled:opacity-60 group w-full'>
              <span className='group-hover:group-enabled:visible invisible mr-2'>*</span>
              <span className='pr-5'>Entrar</span>
            </button>
            <span>ou</span>
            <GoogleLoginButton />
            <Link href="/signin" className='text-sm text-blue-600 cursor-pointer mt-5 underline'>Não tem uma conta?</Link>
          </div>
        </div>
      </TripleBorder>
    </div>
    ...
    <div className="pt-8 grid grid-rows-3 w-full h-full">
      ...
      <TripleBorder className="row-span-2" borderColor="gray-light">
        <div className="w-full h-full md:p-5 md:pt-7 p-3 pt-5 flex flex-col gap-6 md:text-base text-xs">
          ...
          <div onClick={() => signOut(auth)} className="group cursor-pointer"><span className="group-hover:visible invisible mr-2">*</span><span>Sair</span></div>
        </div>
      </TripleBorder>
    </div>
  )
